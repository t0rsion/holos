...
fn params(
    ...
    p.factorization = match factorization {
        "auto" => GraphFactorization::Auto,
        "off" => GraphFactorization::Off,
        "force" => GraphFactorization::Force,
        value => {
    ...
    p.collapse_schedule = match collapse_schedule {
        "serial" => CollapseSchedule::Serial,
        "ordered" => CollapseSchedule::Ordered,
        "rounds" => CollapseSchedule::Rounds,
        "adaptive" => CollapseSchedule::Adaptive,
        value => {
            return Err(PyValueError::new_err(format!(
    ...
    let objective = match collapse_objective {
        "h1" => CollapseObjective::H1,
        "h2" => CollapseObjective::H2,
        value => {
            return Err(PyValueError::new_err(format!(
    ...
    if (collapse_objective != "h2" || collapse_work_limit.is_some())
        && p.collapse_schedule != CollapseSchedule::Adaptive
    {
        return Err(PyValueError::new_err(
