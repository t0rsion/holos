...
## Preregistration: weighted intervention study

`v20_intervention_bench.py` compares certified branch-and-bound search with
an exact flat subset search. The frozen corpus is
`v20_intervention_corpus.toml`, version 1, dated 2026-08-25. No corpus entry
had run when this section was written.

Each graph is a disjoint union of cross-polytope boundaries and isolated
vertices. Each boundary contributes one named class in H1, H2, or H3. One
candidate edge fills each boundary. Lower-sorted candidate edges join only
isolated vertices and cannot kill a named class. Costs are positive integers.
One plan must kill every named class.

The certified arm uses the public intervention builder. The flat arm checks
every candidate subset up to the edit limit. It computes an edited cohomology
space once per subset and tests every named class against one restriction map.
This sharing gives the flat arm less repeated work than separate scenario
evaluation would require. Both arms must return the same unique minimum-cost
plan.

The exact result is positive only if every entry has the expected rank, edit
count, cost, and disjoint root blocker count. The search result is positive
only if its distinct oracle calls are at most half the subsets checked by the
flat arm. The timing result is positive only if certified search is at least
1.5 times faster in every confirmation entry. The proof result requires an
independently accepted artifact smaller than 64 KiB in every entry. The
confirmation set must cover H1, H2, and H3.

The runner alternates certified and flat arm order across five timed
repetitions. Fixture construction, target discovery, initial correctness
checks, and artifact encoding stay outside the clocks. The screen manifest
binds the commit, corpus, binary, and every screen entry before confirmation
can start.

The first version 1 screen exposed a runner error at commit `70315e3`. The
runner applied the confirmation timing rule to the screen, although the
registered rule reads confirmation entries only. The exact, search, proof,
and dimension checks passed. The runner now records the screen timing as not
applicable. The corpus and decision rules did not change. The failed screen
supports no claim.

## Preregistration: graded-interface study

`v14_graded_bench.py` compares dimension-generic separator composition with
a materialized parent reduction. The frozen corpus is
`v14_graded_corpus.toml`, version 2, dated 2026-08-24. No version 2 entry had run
when this section was written.

Each graph contains octahedron boundaries joined at one vertex. Every atom
contributes one essential H2 class. Both policies process the same versions
and independently check the H0, H1, and H2 diagrams. The coverage result is
positive only if both policies retain one H2 class per atom.

The structural result requires a composed root with no materialized scope
above one atom. The baseline must materialize the complete graph. The update
result requires at least a 2.0 times median speedup in every confirmation
entry. The proof result requires at least 1.5 times compression in every
confirmation entry. Generation, initial compilation, proof construction,
correctness checks, and one warm-up remain outside the clocks.

Version 1 reached the screen only. Its second materialized arm showed steep
time and retained-state growth, so the run was stopped without a record.
Version 2 reduces the atom and trajectory counts. The proof records report
sparse term counts so this cost remains visible.

The first version 2 run exposed a producer and checker bug before release.
Absent sparse edges entered the explicit complex at positive infinity. The
diagrams matched, but the certificate sizes and times were invalid. The fix
kept the frozen corpus unchanged and added an infinite-threshold regression.

## Preregistration: separator-interface study

`v13_interface_bench.py` compares exact separator composition with a
materialized parent reduction. The frozen corpus is
`v13_interface_corpus.toml`, version 1, dated 2026-08-24. Screen seeds start
at 91001. Held-out confirmation seeds start at 101001. No corpus entry had
run when this section was written.

Each graph contains complete weighted atoms that meet in one edge. That edge
enters at filtration value zero, so it is a simplex at every filtration
value. The compose arm stores reductions only for the atoms. The materialize
arm uses `InterfacePolicy::Materialize` and also stores each parent reduction.
Both arms receive the same graph versions. Every version must have the same
exact diagram. The final canonical class spaces must also agree.

The structural result is positive only if every compose entry has a composed
root and no materialized scope larger than one atom. Each paired baseline
must materialize the complete graph scope. This rule checks the claimed
execution structure, not only the output.

The update result is positive only if the compose arm's median cumulative
update time is at least 1.5 times faster in every confirmation entry. The
proof result is positive only if its complete warm proof stream is at least
1.5 times smaller in every confirmation entry. Checker time, fresh compile
time, branch time, and column counts are descriptive.

Each binary arm alternates warm update and fresh compilation order across
five timed repetitions. The runner alternates compose and materialize process
order across entries. Generation, initial compilation, proof construction,
correctness checks, and one warm-up stay outside the clocks. The screen
manifest binds the commit, corpus, binary, and every screen entry before
confirmation can start.

## Preregistration: self-adjusting program study

`v11_dynamic_bench.py` measures dependency-frontier repair, parallel
alternatives, and unified proof checking. The frozen corpus is
`v11_dynamic_corpus.toml`, version 3, dated 2026-08-24. Screen seeds start at
51001. Held-out confirmation seeds start at 61001.

Versions 1 and 2 reached the screen only. Version 1 omitted exact class
correspondence from the clean arm. Version 2 added it to both arms. That
screen showed that correspondence dominated state maintenance on the larger
entry. Version 3 uses `CorrespondenceMode::Omit` in the repair arm and omits
correspondence from the clean arm. Both arms return the exact current diagram
and canonical class spaces. Exact correspondence remains the default public
update mode and has separate correctness gates. No confirmation entry had
run. The earlier screen records are superseded and support no claim.

Each repair step swaps adjacent filtration weights in one cyclic atom. The
program must retain and reduce at least one reduction column. The comparison
compiles a clean checked program at each graph. Both arms omit cross-state
correspondence. Exact diagrams and canonical class spaces must agree at every
step. The registered repair result is positive only if every confirmation
entry is at least 1.5 times faster and retains more columns than it reduces.

Each branch arm starts independent alternatives from one checked program.
The serial arm uses one worker. The parallel arm uses the registered branch
count as its worker budget. The results must agree exactly and remain in input
order. A median confirmation speedup of at least 1.25 is a positive branch
result. This rule is separate from the repair decision.

The proof arm builds one `HOLOSPF` DAG for the complete repair trajectory.
The `holos-tda-check` crate verifies it without linking to `holos-tda`.
The registered structural result is positive only if every confirmation
entry stores fewer unique reduction nodes than references and reuses at least
one previously checked weighted reduction. Verification time and proof size
are descriptive. The study does not compare proof checking with an external
proof system.

The runner alternates repair and clean-compilation order across five timed
repetitions. It runs one untimed warm-up. Generation, update search, initial
compilation, proof construction, and correctness checks stay outside those
clocks. The screen manifest binds the commit, corpus, binary, and every screen
entry before confirmation can start.

## Preregistration: compositional program study

`v10_program_bench.py` measures version 0.10 on complete weighted atoms that
share one articulation vertex. The frozen corpus is
`v10_program_corpus.toml`, version 1, dated 2026-08-24. Screen seeds start at
31001. Held-out confirmation seeds start at 41001.

Each entry measures two pairs. The accepted-evaluation pair changes every
atom by a different additive offset. Each atom keeps its edge order, but the
complete graph crosses its global edge order. The program checks its compiled
guards and evaluates the diagram. The comparison recomputes the exact
diagram. The registered result is positive only if every confirmation entry
is at least 2.5 times faster and recovers compilation within its trajectory.

The repair pair crosses a result-sensitive guard in exactly one atom per
step. The program must report one touched and one rebuilt atom. Its comparison
recomputes both the diagram and canonical H1 classes. The runner reports the
median as faster at or above 1.05, in the parity band from 0.95 through 1.05,
or slower below 0.95. This arm does not share the positive decision rule. A
local work reduction does not imply a latency reduction.

The runner alternates arm order across five timed repetitions. Generation,
update search, compilation, and one warm-up stay outside the update clocks.
Every step must agree bit for bit with its comparison. The screen manifest
binds the commit, corpus, binary, and screen entries before confirmation can
start.
...
## Preregistration: version 0.8 sparse-first study

`v08_bench.py` measures exact point construction and graph factorization. Its
corpus is `v08_corpus.toml`, version 1, dated 2026-08-24. The runner writes a
screen record first. It will not run the confirmation set until the screen
manifest binds the commit, both binary hashes, the unchanged corpus bytes,
and the generated input bytes for every screen entry.

Two engineering probes preceded this protocol. Both used existing version
0.7 corpus files on this machine. One was a 2,400-point cube at threshold
0.042982980616541044. The other was a 3,200-point cube at threshold
0.2540856478201759. They checked whether the point constructor justified a
registered study. They chose the two point-density regimes in this corpus.
Their timings are not evidence for a release claim.

Point entries pass the same point file and finite threshold to two release
binaries. The control is version 0.7 at commit `60bf8b49bcaa`. It builds the
condensed distance matrix. The candidate is version 0.8 with automatic point
construction. The runner compares their diagrams exactly before it keeps a
timing.

Block entries use a chain of weighted complete graphs. Consecutive blocks
share one articulation vertex. The off, automatic, and forced factorization
arms read the same triplet bytes. Their diagrams must agree exactly.

Each arm runs once before five or more timed repetitions. The record reports
the median, interquartile range, minimum, maximum, and largest peak RSS. Timed
commands use CPUs `0-3,12-15` by default. This names four physical cores and
their siblings on the registered machine.

The record also includes the CPU, platform, Python and Cargo versions, binary
hashes, input hashes, exact commands, and the three registered decisions. A
preferred-arm regression is separate from the two positive-result rules.

The confirmation interpretation is fixed:

- The point result is positive if both point entries are at least 1.5 times
  faster than version 0.7 and use less peak memory.
- The factorization result is positive if automatic factorization is at least
  1.25 times faster than the off arm. Automatic must also take no more than
  1.05 times the forced-arm time.
- A preferred arm below 0.95 times its control on any confirmation entry is a
  regression. The public release notes must name it.
- Explain and prove profiles are correctness features. This study does not
  use their cost to grade the compute profile.

Run the study from a clean tree. `V07_BIN` must name the preserved control
binary.

```sh
CARGO="cargo +1.92" V07_BIN=/path/to/holos-0.7 \
    ./benchmarks/v08_bench.py
CARGO="cargo +1.92" V07_BIN=/path/to/holos-0.7 \
    ./benchmarks/v08_bench.py --confirm
```

## Preregistration: version 0.16 durable-composition study

`v16_distributed_bench.py` measures ordered composition from a local
content-addressed store. Its corpus is `v16_distributed_corpus.toml`, version
1, dated 2026-08-25. Every shard contains a common chordless four-cycle. The
separator therefore has one essential H1 class.

The clean arm starts after the shard artifacts are in a fresh store. It folds
one shard at a time and publishes one manifest. The recovery arm removes that
manifest, keeps the durable fold records, and repeats the same content-bound
job. The checker arm reloads objects by SHA-256 id and independently replays
every child-core union. The control builds one complete graded reduction on
the materialized graph.

The runner first writes a complete screen manifest that binds the commit,
corpus bytes, and screen entries. The confirmation run rejects a missing or
different manifest. Each arm uses at least five repetitions on CPUs
`0-3,12-15` by default.

The confirmation result is positive only if every entry meets all four
rules:

- The distributed diagram equals complete persistence and has the expected
  essential H1 class.
- Recovery reuses every fold and has a lower median time than a clean fold.
- The largest accumulator-plus-shard artifact is smaller than all shard
  artifacts together.
- Relative cancellation reduces the total shard cell count.

Clean composition, independent checking, and complete reduction times are
descriptive. They do not grade a speed claim.

```sh
CARGO="cargo +1.92" ./benchmarks/v16_distributed_bench.py
CARGO="cargo +1.92" ./benchmarks/v16_distributed_bench.py --confirm
```

## Preregistration: version 0.17 exact-class study

`v17_cohomology_bench.py` checks fixed-scale class relations, affine events,
and finite-candidate interventions. Its corpus is
`v17_cohomology_corpus.toml`, version 1, dated 2026-08-25. The entries cover
H1, H2, and H3 over Z/2, Z/3, and Z/5.

Each entry uses the flag complex of a cross-polytope boundary, with two extra
isolated vertices. The complex has one class in the requested dimension.
Adding either antipodal edge fills the sphere and kills that class. The first
declared candidate joins the isolated vertices and does not kill it. The
second candidate is the filling edge.

The runner writes a screen manifest that binds the commit, corpus bytes, and
screen entries. The confirmation run rejects a missing or different
manifest. Each arm uses at least five repetitions on CPUs `0-3,12-15` by
default.

Every confirmation entry must meet all four rules:

- Canonical fixed-scale cohomology and full persistence both report rank one.
  The filled complex and its relation report rank zero. The identity relation
  is an isomorphism.
- The exact affine schedule has one threshold crossing. Its open-cell ranks
  change from one to zero.
- Cardinality search rejects the first candidate and proves the second is an
  optimal one-edge intervention. The independent checker repeats the search.
- The self-contained intervention artifact is smaller than 4,096 bytes.

All times are descriptive. This study does not grade a speed claim because
the fixed-scale basis and full persistence return different objects.

```sh
CARGO="cargo +1.92" ./benchmarks/v17_cohomology_bench.py
CARGO="cargo +1.92" ./benchmarks/v17_cohomology_bench.py --confirm
```
