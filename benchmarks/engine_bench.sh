...
#
# The storage axis is not wired in here. A tree arm also takes
# `--dense-storage auto|compact|square`, and crates/engine-bench takes the
# same flag, so both storage forms can be timed on one entry. Adding them
# as configurations doubles every dense run and changes the shape of the
# record, so the storage tuning runs outside this script and this script
# times the shipped rule alone. To wire it in, give config_input two more
# names, forced-dense-compact and forced-dense-square, each passing
# `--engine dense` with its own `--dense-storage`.
#
...
and no protocol gate. It reports timings, peak RSS, and agreement; the
...
while IFS=$'\t' read -r id stratum input family generator n coord_dim param max_dim tau modulus seed headline competitor collapse driver_modes; do
    ...
    # A collapsed entry is a sparse input whatever it grew from: the
    # reduced graph is written as triplets and no dense file of it exists.
    file_kind="$input"
    if [[ "$collapse" == yes ]]; then
        file_kind=graph
    fi
    ...
    # One ripser run per configuration, on the same file the arms read.
    entry_void=0
    all_configs=""
    for i in "${!ARM_LABEL[@]}"; do
...
# The tables and the stratum summary. The record is a set of tab-separated
# rows; this step turns them into markdown and computes the medians. It
