#!/usr/bin/env python3
"""Run the registered version 0.9 persistence-atlas study.

Usage: v09_atlas_bench.py [--confirm]

The screen must finish before confirmation. The screen manifest binds the
commit, benchmark binary, corpus, and screen entries. Each binary invocation
compares atlas reuse with full exact reduction over the same affine weight
trajectory. It rejects any bit-level diagram mismatch before printing a
timing record.
...
Environment:
  ...
  ATLAS_AFFINITY  CPUs for the study. The default is ``0-3,12-15``.
  ALLOW_DIRTY     Set to 1 to record changes outside this study's generated
                  records. Such a record is void for release claims.
"""
...
CORPUS = HERE / "v09_atlas_corpus.toml"
MANIFEST = HERE / "results_v09_atlas_screen_manifest.txt"
...
def fields(line):
    ...
    for item in line.split():
        ...
        values[key] = value
    if values.get("format") != "holos-atlas-bench-v1":
...
def main():
    ...
    affinity = os.environ.get("ATLAS_AFFINITY", "0-3,12-15")
    cargo = os.environ.get("CARGO", "cargo").split()
    subprocess.run(
        [*cargo, "build", "--release", "-p", "atlas-bench", "--locked"],
        cwd=ROOT,
        check=True,
    )
    binary = ROOT / "target/release/atlas-bench"
    dirty_lines = text(["git", "status", "--porcelain"], cwd=ROOT).splitlines()
    source_dirty = [
        line for line in dirty_lines
        if "benchmarks/results_v09_atlas_" not in line
    ]
    ...
    screen_bindings = {
        entry["id"]: entry_binding(entry) for entry in corpus["screen"]
    }
    expected_manifest = [
        f"commit={commit}",
        ...
        *[
            f"entry={entry_id}:{binding}"
            for entry_id, binding in screen_bindings.items()
        ],
    ]
    if args.confirm:
    ...
    for entry in corpus[table]:
        ...
        record = fields(result.stdout.strip())
        records.append((entry, benchmark, record))

    passed = all(
        float(record["speedup"]) >= 3.0
        and int(record["break_even_steps"]) <= entry["steps"]
        for entry, _, record in records
    )
    raw = HERE / f"results_v09_atlas_{table}.txt"
    markdown = HERE / f"results_v09_atlas_{table}.md"
    metadata = [
        f"commit={commit}",
    ...
    md = [
        ...
        f"- Set: {table}",
        f"- Repetitions: one warm-up and {reps} counterbalanced timed runs",
    ...
    for entry, _, record in records:
        md.append(
            f"| {entry['id']} | {entry['n']} | Z/{entry['modulus']} | {entry['steps']} | "
            f"{int(record['compile_ns']) / 1e6:.3f} | "
            f"{int(record['reuse_ns']) / 1e6:.3f} | "
            f"{int(record['recompute_ns']) / 1e6:.3f} | "
            f"{float(record['speedup']):.2f}x | {record['break_even_steps']} |"
        )
    markdown.write_text("\n".join(md) + "\n")
    if not args.confirm:
