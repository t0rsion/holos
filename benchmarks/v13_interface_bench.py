#!/usr/bin/env python3
"""Run the registered version 0.13 separator-interface study.

Usage: v13_interface_bench.py [--confirm]

The screen must finish before confirmation. Each entry uses complete weighted
atoms joined by one zero-filtration edge. The compose arm certifies that edge
as a simplex interface. The baseline retains a reduction over every parent
scope. Both arms process identical updates and check every exact diagram.
...
CORPUS = HERE / "v13_interface_corpus.toml"
MANIFEST = HERE / "results_v13_interface_screen_manifest.txt"
...
def main():
    ...
    source_dirty = [
        ...
        if "benchmarks/results_v13_interface_" not in line
    ]
    ...
    update_pass = all(
        ratio(baseline["warm_ns"], compose["warm_ns"]) >= 1.5
        for _, pair in records
        for compose, baseline in [(pair["compose"][1], pair["materialize"][1])]
    )
    proof_pass = all(
        ratio(baseline["warm_proof_bytes"], compose["warm_proof_bytes"])
        >= 1.5
        for _, pair in records
        for compose, baseline in [(pair["compose"][1], pair["materialize"][1])]
    )

    raw = HERE / f"results_v13_interface_{table}.txt"
    markdown = HERE / f"results_v13_interface_{table}.md"
    metadata = [
        f"commit={commit}",
    ...
    md = [
        "<!-- Generated by benchmarks/v13_interface_bench.py. Do not edit. -->",
        "",
        f"- Commit: `{commit}`",
        ...
        "- Structural rule: composed root, no parent scope above one atom, and a full-scope materialized baseline",
        f"- Structural decision: {'pass' if structural_pass else 'fail'}",
        "- Update rule: composed median time is at least 1.5 times faster in every entry",
        ...
        "",
        "| entry | atoms | vertices | field | largest interface | update compose/materialize (ms) | speedup | proof compose/materialize (MiB) | compression | check compose/materialize (ms) |",
        "|:--|--:|--:|:--|--:|--:|--:|--:|--:|--:|",
    ]
    for entry, pair in records:
        ...
        md.append(
            ...
            f"{int(compose['warm_proof_bytes']) / (1 << 20):.3f}/{int(baseline['warm_proof_bytes']) / (1 << 20):.3f} | {proof_compression:.2f}x | "
