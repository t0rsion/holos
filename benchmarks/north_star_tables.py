...
script to turn them into markdown. Without --text the output is markdown; with
--text it is one key=value line per figure, for the .txt log. This step
computes medians, ratios, the noise band, and the grades. It measures nothing.
...
def markdown(study, serial, multicore):
    ...
    out.append("number of descriptive entries it left out. This table comes before any")
    out.append("overall median on purpose: a median over unlike regimes hides the")
    out.append("regime that lost.")
    out.append("")
    ...
    out.append("")
    out.append(f"H2 and H3, both at {cores} physical cores, on the same entries. The")
    out.append("ratio says how much of H3's multicore position came after H2.")
    out.append("")
    rows = []
    for row in study.scaling:
